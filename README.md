...
## [통합 영상](https://youtube.com/shorts/IZrWX7b9LR8?si=yA0GcrZOKLtWjx4z)
...
<table align="center">
 ...
 1인 가구가 증가함에 따라 혼자서도 식단 관리, 운동 등을 할 수 있게 도와주는 관련 헬스케어 서비스들이 증가하고 있다. 하지만 기존 식단 관리 서비스는 사용자가 직접 데이터를 입력해야 하는 번거로움이 있고, 운동 관리 서비스는 실시간으로 자세 피드백을 받기 어렵다는 한계가 있다. 또한, 혼자 사는 사람들이 낙상과 같은 사고를 겪을 경우 신속하게 대처하기 어렵다는 문제도 있다. 이러한 점들을 해결하기 위해 NahonLab팀은 딥러닝 기술을 활용하여 운동, 식단, 건강 관리 기능을 통합한 서비스를 개발하였다.
...
| **분류**       | **기능**                                | **설명**                                                                                 |
|:----------------:|---------------------------------------|-----------------------------------------------------------------------------------------|
| **식단**       | 음식 영양 성분 분석 기능       | • 업로드한 식단 사진을 분석하여 영양 성분 및 칼로리를 계산하여 제공 및 저장<br>• 음식 양 수정 기능  |
...
## 4-2. 프로젝트 리뷰 및 개선 방안
### 일반 알고리즘을 활용한 구현
- 음식 양 측정, 식단 및 운동 추천, 운동 시 관절의 부정확한 자세를 구별하려는 시도를 딥러닝 기술로 구현하고자 함.
...
### 클래스의 제한으로 인한 분석 한계
- 음식 모델의 클래스가 11개로 제한되어 세분화된 분석이 어려움.
- 위험 상황 모델 또한 낙상/정상 상태만 구별 가능하여 다양한 상황을 처리하기에는 부족한 점이 한계로 작용.
### 향후 개선 방안
